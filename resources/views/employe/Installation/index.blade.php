...
@section("content")
    ...
    <form class="form" action="#" method="POST"
        ...
        <div class="container">
            ...
            <div class="card">
                <div class="card-header" style="color:white; background-color:#97195a;">Create New Intallation</div>
                <div class="card-body">
                    <input type="hidden" id="numberofRow" name="numberofRow" value="{{$order->Installation->count()}}">
                    <div id="content">
            ...
            <span class="btn btn-success btn-block mt-3" onclick="createNewInstalltion()">Create New Installation <i
...
@push('custom-scripts')
    <script>
        function addNewElement() {
            ...
            let path = "{{route('employe.Installation.getRow')}}";
        ...
        function createNewInstalltion() {
            let form = $('.form');
            let path = "{{route('employe.Installation.createNewInstallation')}}";
            let data = new FormData(form[0]);

            ajaxRequest(path, data, function (response) {
                if (response.status == 200) {
                    swal("create New Installation!", response.messsage, "success");
                    $('#content').html('');
                    $('#name').val('');
                    $('#phone').val('');
                    $('#total_invoice').val('');
