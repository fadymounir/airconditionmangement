@component('dashboard.layouts.tools.modalNormal')
    ...
    @slot('modalBody')
        ...
        @component('dashboard.layouts.components.inputTag')
            @slot('label','الوصف  بالعربي')
            @slot('icon','fas fa-shopping-cart')
            @slot('name','name_ar')
        @endcomponent

        @component('dashboard.layouts.components.inputTag')
            @slot('label','الوصف بالانجليزي')
            @slot('icon','fas fa-shopping-cart')
            @slot('name','name_en')
        @endcomponent
...
@push('custom-scripts')
    <script>
        function showModal(type, id = 'null') {
            if (type == "add") {
                $('#modalTitle').text('اضافة وصف جديدة');
                $('#actionType').val('add');
                $('#name_ar').val('');
                $('#name_en').val('');
                ...
                let path = "{{route('adminpanel.problemsDescribingOrders.getRecord')}}";
                let data = new FormData();
                data.append('id', id);
                ajaxRequest(path, data, function (response) {
                    $('#modalTitle').text('تعديل بيانات الوصف');
                    $('#actionType').val('edit');
                    $('#cityId').val(id);
        ...
        $('#submit').click(function () {
            let path = "{{route('adminpanel.problemsDescribingOrders.addEdit')}}";
