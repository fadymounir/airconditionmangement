@component('dashboard.layouts.tools.modalNormal')
    ...
    @slot('modalBody')
        ...
        @component('dashboard.layouts.components.inputTag')
            @slot('label',' المشتريات  بالعربي ')
            @slot('icon','fas fa-shopping-cart')
            @slot('name','name_ar')
        @endcomponent

        @component('dashboard.layouts.components.inputTag')
            @slot('label',' المشتريات بالانجليزي')
            @slot('icon','fas fa-shopping-cart')
            @slot('name','name_en')
        @endcomponent
...
@push('custom-scripts')
    <script>
        function showModal(type, id = 'null') {
            if (type == "add") {
                $('#modalTitle').text('اضافة مشتريات جديدة');
                $('#actionType').val('add');
                $('#name_ar').val('');
                $('#name_en').val('');
                ...
                let path = "{{route('adminpanel.purchases.getRecord')}}";
        ...
        $('#submit').click(function () {
            let path = "{{route('adminpanel.purchases.addEdit')}}";
